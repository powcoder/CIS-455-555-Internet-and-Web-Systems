...
public class CrawlerTest {
    /**
     * Test if it can extract links from html content
     */
    @Test
    public void extractLinkTest() throws Exception{
        ...
        Crawler crawler = new Crawler(null, null, 0, 0);
        List<String> list = crawler.extractLinks(content);

        Assert.assertEquals(13, list.size());
    }

    @Test
    public void extractParentDirTest() throws Exception{
        Crawler crawler = new Crawler(null, null, 0, 0);
        String urlParent1 = crawler.extractParentDir(new URLInfo("http://www.test.com/test/index.html"));
        Assert.assertEquals(urlParent1, "www.test.com:80/test");
        String urlParent2 = crawler.extractParentDir(new URLInfo("http://www.test.com/test/"));
     ...
     */
    @Test
    public void isOKtoCrawlTest() throws Exception{
        ...
        //disallow everything
        info.addDisallowedLink("cis455crawler", "/");
        robotsMap.put("www.test.com:80", info);
        crawler.setRobotsMap(robotsMap);
        boolean result = crawler.isOKtoCrawl("http://www.test.com", 80, false);

        Assert.assertEquals(false, result);
    }

    @Test
    public void isOKtoParseTest() throws Exception{
        StorageInterface db = mock(StorageInterface.class);
        Crawler crawler = new Crawler("http://www.test.com", db, 0, 0);
        HashMap<String, RobotsTxtInfo> robotsMap = new HashMap<>();
        RobotsTxtInfo info = new RobotsTxtInfo();
        //disallow everything
        info.addDisallowedLink("cis455crawler", "/test");
        robotsMap.put("www.test.com:80", info);
        crawler.setRobotsMap(robotsMap);
        boolean result = crawler.isOKtoParse(new URLInfo("http://www.test.com/test"));
